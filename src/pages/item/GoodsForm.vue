            ...
            <v-form v-model="valid" ref="basic">
              <v-layout row>
                ...
                <v-flex offset-xs2 xs5>
                  <v-select label="所属品牌" v-model="goods.brandId" :items="brandOptions" dense
                            autocomplete
                            item-text="name" item-value="id" :rules="[v => !!v || '品牌不能为空']"
                            required/>
                </v-flex>
              </v-layout>
              ...
              <v-text-field label="包装清单" v-model="goods.spuDetail.packingList" :counter="1000"
                            multi-line :rows="3"/>
              <v-text-field label="售后服务" v-model="goods.spuDetail.afterService" :counter="1000"
                            multi-line :rows="3"/>
              <v-layout row>
                <v-flex xs3>
                </v-flex>
            ...
            <div v-for="(t,i) in template" :key="i">
              ...
              <div class="px-5" v-if="t.options.length <= 0">
                ...
                <v-layout row>
                  ...
                  <v-spacer/>
                  <v-flex xs1>
                    <v-btn icon small @click="deleteOption(i,0)"><i class="el-icon-delete"></i>
                    </v-btn>
                  </v-flex>
                </v-layout>
                <!--其他项-->
                <v-layout row v-for="i2 in skuTemplate[i].options.length" :key="i2">
                  ...
                  <v-spacer/>
                  <v-flex xs1>
                    <v-btn icon small @click="deleteOption(i,i2)"><i class="el-icon-delete"></i>
                    </v-btn>
                  </v-flex>
                </v-layout>
              </div>
